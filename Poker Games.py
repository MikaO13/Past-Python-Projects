...
winningHandsVals = {"Royal Flush":1000, "Straight Flush":200, "Four of a Kind":50, "Full House":11, "Flush":8, "Straight":5, "Three of a Kind":3, "Two Pair":2, "Pair 10+":1}
...
class Chips:
    ...
    def win_bet(self, winType):
        self.total += self.numBets * self.lirbet * winningHands[winType]
    
    def lose_bet(self):
        self.total -= self.lirbet * self.numBets
...
def texasholdem(first_time=True, pick_players=True):
    global forbiddenames
    global pot
    global currentBet
    global startingBet
    ...
    if first_time:
        startingPlayer = 0
        print("Hello and Welcome to Mika's Texas Hold Em' Table!")
        print('Other games by Mika: Blackjack, Pass the Pig, Hangman, Anagram, Go Fish, War, Uno, Connect 4\n')
        if 'y' in input('Would you like to hear the rules? ').lower():
            print('This game is a variation of the classic betting card game, poker.'
                  ...
                  '\nI will refer you to this website - https://palapoker.com/texas-holdem/ - to learn the rules.'
    ...
    for rnd in ['begin','flop','turn','river']:
        ...
        if rnd == 'begin':
            ...
            players[startingPlayer].chips.bet(2)
            players[startingPlayer+1].chips.bet(5)
            startingBet = 5
            currentBet = 5
            ...
            for player in players:
                if player.chips.inRound == True:
                    for player in players:
                        player.add_card(rivercard, True)
                        player.sort(player.fullHand)
                else:
                    donePlayers += 1
            printCommunityCards(communityCards)
    ...
    for player in players:
        player.winningHand, player.winningType = check_win_nonlir(player.fullHand)
        ...
        while len(tieWinners) != 1:
            ...
            if len(tieWinners) > 1:
                ...
                for winner in tieWinners:
                    ...
                    winner.winningHand, throwawayVar = check_win_nonlir(winner.winningHand)
            winners = tieWinners
    ...
    if 'y' in play_again.lower():
        ...
        if 'y' in repick_players.lower():
            startingPlayer += 1
            texasholdem(False, True)
        else:
            texasholdem(False, False)
    else:
        print("Thank you for playing at Mika's Texas Hold Em' Table!\nPlease play again and tell your friends!")
...
def letitride(first_time=True, pick_players=True):
    ...
    for player in players:
        player.chips.lirbet = player.take_bet()
    ...
    for rnd in range(2):
        for player in players:
            ...
            if choice == 'pull':
                ...
                continue
            print("{} has {} bets of {} chips left.\n".format(player.name, str(player.chips.numBets), str(player.chips.lirbet)))
    ...
    for player in players:
        player.winningHand, player.winningType = check_win_lir(player.fullHand)
        winningCards = ""
        for card in player.winningHand:
            winningCards += card.rank +' of '+ card.suit
            if player.winningHand.index(card) != len(player.winningHand) - 1:
        ...
        print("{} has a {}, with cards {}.".format(player.name, player.winningType, winningCards))
        if player.winningType in winningHandsVals:
            print("With a {} bet(s) of {}, {} earns {} chips.".format(str(player.chips.numBets), str(player.chips.lirbet), player.name, str(player.chips.numBets * player.chips.lirbet * winningHandsVals[player.winningType])))
            player.chips.win_bet(player.winningType)
        else:
            print("With a {} bet(s) of {}, {} loses {} chips.".format(str(player.chips.numBets), str(player.chips.lirbet), player.name, str(player.chips.lirbet)))
...
def play_poker():
    games = ['the', 'lir', 'scs']
    help_message = "When being asked to play, you may enter in the acronym of the game you wish to play, the command 'help' to see the options of games to play (along with this message), or 'quit' to quit the game."
    games_message = "The games that are available to play currently are Texas Hold Em', Let It Ride, and Seven Card Stud."
    print("Hello, and welcome to Mika's Poker Tables!")
    print("Here, you will be able to play a variety of poker games.")
    print(games_message)
    print(help_message)
    ...
    while True:
        ...
        if choice in games:
            ...
            print("Thank you for playing at Mika's Poker Tables, and we hope to see you again soon!")
            break
        else:
            print("That is not a valid option. If you need help, enter the command 'help'.")