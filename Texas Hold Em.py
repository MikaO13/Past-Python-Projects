...
def straight_flush(hand): 
    # has a loophole rn bc straight might not get all of the same suit
    # but it might have cards w the same suit in a straight anyways
    winning = straight(hand)
    if winning:
        suit = winning[0].suit
        check = True
        for card in winning:
...
hand = [Card('Diamonds', 'King'), Card('Diamonds', 'Three'), 
        Card('Diamonds', 'Ace'), Card('Diamonds', 'Six'), 
        Card('Diamonds', 'Queen'), Card('Diamonds', 'Five'), 
        Card('Diamonds', 'Two')]
...
def texasholdem(first_time=True, pick_players=True):
    ...
    for player in players:
        ...
        print(player) #get rid of later - only here for debugging
        ...
        while len(tieWinners) != 1:
            ...
            for winner in winners:
                ...
                if winner.winningHand == []:
                    ...
                    if values[winner_high_card] < var_high_card:
                        var_high_card = winner_high_card
                        tieWinners = [winner]
                    elif values[winner_high_card] == var_high_card:
